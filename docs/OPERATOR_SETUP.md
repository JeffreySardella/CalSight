...
| Groq / Gemini / OpenRouter API keys | backend `.env` on LXC 100 | Create the new key in the provider console, swap in `.env`, restart backend, then delete the old key in the console. The multi-provider fallback keeps Ask AI alive if one provider's swap goes wrong. |
