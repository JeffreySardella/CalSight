...
| `mv_crashes_by_day` | ~530,000 | County × calendar day with `crashes, killed, injured, dui_crashes`; migration `e15a88b36527`, added 2026-09. Serves `/api/holidays`. Optional — the endpoint returns an empty list until the first refresh |
...
There are 13 materialized views as of 2026-09-19 (see the row-count table). The four original StatsPage views are not interchangeable — pick the smallest one that supports your filters:
