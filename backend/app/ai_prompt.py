...
You have tools to query the database. Use them to get real data before answering. Do not guess or invent statistics — if you need a number, call a tool. Leave out a tool parameter you do not need; never pass null.
...
- Road-user mode: people injured or killed as pedestrian / cyclist / motorcyclist / occupant (2016+)
- Exposure: CARB EMFAC vehicle miles traveled per county per year, for per-100M-VMT rates
- School proximity: crashes within 500 ft of each K-12 school
- Census tracts: per-tract crash counts alongside CalEnviroScreen burden percentiles
...
- For "who is being killed" questions (pedestrians, cyclists, motorcyclists, vehicle occupants) call get_mode_breakdown — it counts PEOPLE, not crashes, covers only people with a recorded injury outcome, and starts in 2016. Do not mix its people counts with crash counts from query_crashes in the same sentence.
- For per-mile-driven rates call get_vmt (crashes and deaths per 100M vehicle miles). Population, licensed drivers and road miles are get_crash_rate's denominators; VMT is the exposure one and answers a different question.
- School-proximity (get_school_crashes) and census-tract (get_tract_burden) figures can only see crashes that carry coordinates — about 37% statewide, and it varies by county and agency. Always report those as located crashes and a floor, never as totals, and quote the coverage number the tool returns.
- get_tract_burden's ces_percentile is a CalEnviroScreen 5.0 cumulative-burden percentile (0 = least burdened in California, 100 = most burdened), a single snapshot with no year-over-year series.
- For rain-and-crash questions (first storm of the season, days since rain, does rain raise crashes) call the first_rain tool — it is the only water-adjacent data you have, and its lift is an association, not causation. You have NO tools for reservoir storage, snowpack, drought or general precipitation; for those, point users to the Water page at https://calsight.org/water — never guess or invent water figures.
...
- Questions about danger, safety or risk ("is it getting more dangerous", "how safe is", "riskiest") are about people killed or seriously injured, not crash counts. Answer with deaths and KSI (killed or seriously injured); most crashes are minor, so crash counts can fall while deaths rise. For pedestrians or cyclists call get_mode_breakdown with mode and by_year=true (people, 2016+).
- Use the latest year with data, not an earlier cut-off. Death records lag six months or more, so the most recent year is incomplete and its deaths will rise: say which years are provisional, and never call a drop in a provisional year an improvement.
- Describe a trend only with a word that matches the numbers you show. A series that falls every year after 2020 has no "rebound"; a rising one is not a "decline". Check the year-by-year direction before you write the sentence.
...
TOOL_DEFINITIONS = [
    ...
    {"type": "function", "function": {"name": "get_mode_breakdown", "description": "People injured or killed by road-user mode: pedestrian, cyclist, motorcyclist (motorcycles and mopeds) and occupant (everyone else riding in a vehicle). Use for 'how many pedestrians were killed', 'cyclists vs motorcyclists', 'Vision Zero / who is dying on the roads' questions. Counts PEOPLE, not crashes, and only people with a recorded injury outcome — so victim_count is people hurt or killed, not everyone present. Victim records are CCRS-only, so mode data STARTS IN 2016 and cannot answer earlier years. Returns four rows (or one per year with by_year) with ksi_count = killed + seriously injured, a status on still-provisional recent years, plus caveat text you must repeat.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide)"}, "years": {"type": "array", "items": {"type": "integer"}, "description": "Crash years, 2016 or later"}, "severity": {"type": "string", "enum": ["Fatal", "Injury", "Property Damage Only"], "description": "The CRASH's severity; the casualty columns still come from each person's own injury outcome"}, "mode": {"type": "string", "enum": ["pedestrian", "cyclist", "motorcyclist", "occupant"], "description": "Only this road user"}, "by_year": {"type": "boolean", "description": "One row per year (with mode: that mode's yearly series) instead of one row per mode. Use for trend questions like 'is walking getting more dangerous'"}}}}},
    {"type": "function", "function": {"name": "get_vmt", "description": "Vehicle miles traveled (CARB EMFAC) per year with crashes and deaths per 100 million VMT — the exposure-based safety rate road-safety work normally uses. Use for 'crashes per mile driven', 'how much do people drive in X', 'is driving getting more dangerous per mile', or any rate question where population or drivers is the wrong denominator. vmt_millions is millions of miles for the whole year on every road; it is NOT Caltrans AADT. A rate appears only for years present in both VMT and crash data — vmt_years_available reports the coverage. Returns up to 20 years plus caveat text you must repeat.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide)"}, "year_start": {"type": "integer"}, "year_end": {"type": "integer"}}}}},
    {"type": "function", "function": {"name": "get_school_crashes", "description": "Crashes within 500 ft of K-12 schools: the totals for the area plus the schools with the most. Use for 'crashes near schools', 'school zone safety', 'which school has the most crashes nearby'. Only crashes that carry coordinates can be placed near a school — about 37% statewide, reported per request as coord_coverage_pct — so these are LOCATED crashes only, a floor and not a total, and proximity is not attribution (not limited to school hours). Returns up to 20 schools plus caveat text you must repeat.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide)"}, "years": {"type": "array", "items": {"type": "integer"}}, "limit": {"type": "integer", "description": "Max schools (default 10, max 20)"}}}}},
    {"type": "function", "function": {"name": "get_tract_burden", "description": "Census-tract crash burden against CalEnviroScreen score — the environmental-justice cut. Returns crashes and crashes per 1,000 residents by CES burden band (ces_percentile 0 = least burdened in California, 100 = most burdened) plus the tracts carrying the most located crashes. Use for 'do disadvantaged neighborhoods see more crashes', 'equity', 'CalEnviroScreen and crashes', 'which neighborhoods/tracts are worst'. Tract counts cover only crashes with coordinates — about 37% statewide, reported per request as coord_share — so they are a floor and not a total; any band pattern is an association confounded by traffic volume, road design and density. Up to 20 tracts plus caveat text you must repeat.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide)"}, "year_start": {"type": "integer"}, "year_end": {"type": "integer"}, "limit": {"type": "integer", "description": "Max tracts (default 10, max 20)"}}}}},
    {"type": "function", "function": {"name": "first_rain", "description": "First measurable rain of the water year and its crash lift, per county and statewide (association, not causation). Use for 'first storm', 'first rain of the season', 'days since it rained', 'do crashes go up when it rains after a dry spell'. Returns the definition (rain threshold, dry-spell minimum, baseline window), the statewide median lift and one water year's roll-up, and for a county its latest (or requested) event plus days since its last measurable rain. small_baseline=true means the percent is noise-prone — say so.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide only)"}, "water_year": {"type": "integer", "description": "Water year (Oct-Sep, named for the year it ends); default: latest"}}}}},
]
