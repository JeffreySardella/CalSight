...
TOOL_DEFINITIONS = [
    ...
    {"type": "function", "function": {"name": "get_mode_breakdown", "description": "People injured or killed by road-user mode: pedestrian, cyclist, motorcyclist (motorcycles and mopeds) and occupant (everyone else riding in a vehicle). Use for 'how many pedestrians were killed', 'cyclists vs motorcyclists', 'Vision Zero / who is dying on the roads' questions. Counts PEOPLE, not crashes, and only people with a recorded injury outcome — so victim_count is people hurt or killed, not everyone present. Victim records are CCRS-only, so mode data STARTS IN 2016 and cannot answer earlier years. Returns four rows plus caveat text you must repeat.", "parameters": {"type": "object", "properties": {"county": {"type": "string", "description": "County name or slug (omit for statewide)"}, "years": {"type": "array", "items": {"type": "integer"}, "description": "Crash years, 2016 or later"}, "severity": {"type": "string", "enum": ["Fatal", "Injury", "Property Damage Only"], "description": "The CRASH's severity; the casualty columns still come from each person's own injury outcome"}}}}},
