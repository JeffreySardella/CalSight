...
COUNTY = 19  # Los Angeles, from the shared seed — the endpoint reads whatever
             # is in storm_events, so the zone map is not part of this test.
...
def test_statewide_fog_day_lift(client, db_session):
    ...
    assert county["county_slug"] == "los-angeles"
    assert county["years"][0]["fog_event_days"] == 3


def test_county_and_year_filters(client, db_session):
    _seed(db_session)
    scoped = client.get("/api/fog-days?county=los-angeles&year=2024").json()
    assert scoped["county"] == "los-angeles"
