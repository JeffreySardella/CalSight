...
| `storm_events` | NOAA event × county (2001+) | `etl.load_storm_events` | NOAA Storm Events Dense Fog and winter rows for the 32 San Joaquin Valley + Sierra counties; keyed to NWS forecast zones, so one event becomes one row per county its zone touches — unique on (source_event_id, county_code). Its `source` column is NOAA's own SOURCE field (who reported the event: "Trained Spotter", "Broadcast Media"), **not** CalSight provenance — unlike `etl_runs.source` everywhere else in this doc |
